...
def build_violations(recs, section_name, year):
    ...
    total=len(recs)
    gA=sum(1 for r in recs if r.get('grade')=='A')
    gB=sum(1 for r in recs if r.get('grade')=='B')
    gC=sum(1 for r in recs if r.get('grade')=='C')
    MC(ks,1,1,8,f'لوحة المخالفات – {section_name}  |  نورة لتعليم القيادة',size=14); RH(ks,1,36)
    MC(ks,2,1,8,f'إجمالي السجلات: {total}  |  السنة: {year}',bold=False,bg=TEAL,size=10); RH(ks,2,20)
    MC(ks,4,1,8,'📊 مؤشرات الأداء الرئيسية',bg=TEAL,size=11); RH(ks,4,26)
    ...
    if total:
        KPI(ks,9,1,'نسبة A',f'{gA/total*100:.1f}%',TEAL2)
        KPI(ks,10,1,'نسبة B',f'{gB/total*100:.1f}%',TEAL2)
    for c,w in enumerate([18,10,10,10,18,10,10,10],1): CW(ks,c,w)
    
    # جدول الإجمالي الشهري
    ...
    for mi,month in enumerate(MONTHS):
        ...
        month_recs=[x for x in recs if (x.get('date') or '').startswith(f'{year}-{str(mi+1).zfill(2)}')]
        mt=len(month_recs); mA=sum(1 for x in month_recs if x.get('grade')=='A')
        mB=sum(1 for x in month_recs if x.get('grade')=='B')
        mC=sum(1 for x in month_recs if x.get('grade')=='C')
    ...
    # شيت البيانات
    ds=wb.create_sheet('سجل المخالفات'); ds.sheet_view.rightToLeft=True
    MC(ds,1,1,8,f'سجل المخالفات – {section_name}  |  نورة لتعليم القيادة',size=14); RH(ds,1,34)
    ...
    def do_POST(self):
        try:
            ...
            if rtype not in builders:
                ...
                self.end_headers()
                self.wfile.write(b'Unknown type')
                return
